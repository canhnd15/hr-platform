...
export async function generateMetadata({
  ...
  if (!tenant || !searchParams.id) {
    return { title: "Job Detail" };
  }
  const job = await getJobByIdForTenant(tenant.id, searchParams.id);
  if (!job) return { title: `Job Detail | ${tenant.company.name}` };
  return {
    title: `${job.title} — ${tenant.company.name}`,
...
export default async function JobDetailPage({ params, searchParams }: Props) {
  ...
  const job = id ? await getJobByIdForTenant(tenant.id, id) : null;
            ...
            <div className="job-overview-grid">
              <div className="overview-item">
                ...
                <div className="overview-text">
                  ...
                  <span className="overview-item-value">{job.location}</span>
                </div>
              </div>
